...
```
## 消息发送与接收实现
在v2.1.0后，将 WebSocket 聊天服务器使用 HTTP 服务器替代，从而实现多对多的消息发送，并减少了部分消息服务器的代码。
...
代码详见 [file_server.dart](lib/utils/file_server.dart)
...
```
同时 web 端需要存储这个 $short_hash 指向的 blob 文件,blob 可能是多个。

代码详见 [chat_controller.dart](lib/app/controller/chat_controller.dart) 的 `sendFileForBroswer` 函数。

**3. 客户端请求 web 端上传这个文件**

还是由于 web 端没有静态部署的能力，所以客户端只有请求 web 端将这个文件上传到指定的位置。
```json
{
    "msgType":"notify",
    "hash":"请求上传的文件消息的hashcode",
    "port":13000,
    "address":["a.a.a.a","b.b.b.b"],
}
```
这段消息可以翻译成一个更好理解的意思：“上传了 $hash 的浏览器，你现在可以将这个 hash 指向的文件上传到 [a.a.a.a b.b.b.b]，端口为 13000。”

**4. web端上传这个文件**

通过第 2 步，web 端能够知道 hash 是不是自己发送过的文件，并且自己存下了这些文件的 blob 引用。

这个时候，依然有一个 IP 筛选的功能，是由 web 端筛选出一个可以正确上传的 url，代码详见 [file_server.dart](lib/utils/file_server.dart)。

筛选规则详见 [chat_controller.dart](lib/app/controller/chat_controller.dart) 的 `getCorrectUrlWithAddressAndPort` 函数。

上传的实现就简单了，dio 本就支持 web。
...
```
`XFile`是一个通用描述文件的对象，可以描述各个平台的文件，是 dart 官方实现的库。
## IP 选择实现
